...
public class CoordinateConversion
{
  ...
  public Coordinate[] utm2LatLon(Coordinate[] utmCoordinates, String latZone, String longZone)
  {
    UTM2LatLon c = new UTM2LatLon();
    return c.convertUTMToLatLong(utmCoordinates,latZone,longZone);
  }
  ...
  public HashMap<String,Object> latLon2UTM(Coordinate[] coordinates)
  {
    LatLon2UTM c = new LatLon2UTM();
    return c.convertLatLonToUTM(coordinates);
  ...
  private class LatLon2UTM
  {
    public String convertLatLonToUTM(double latitude, double longitude)
    {
      ...
      UTM = longZone + " " + latZone + " " + (Math.round(_easting*10)/10) + " "
          + ( Math.round(_northing*10)/10);
    ...
    public HashMap<String,Object> convertLatLonToUTM(Coordinate[] coordinates)
    {
    	double latSum = 0;
    	double lonSum = 0;
    	
    	for(Coordinate c : coordinates) {
    		latSum = latSum + c.getOrdinate(0);
    		lonSum = lonSum + c.getOrdinate(1);
    	}
    	
    	double meanLat = latSum/coordinates.length;
    	double meanLon = lonSum/coordinates.length;
    	
    	validate(meanLat,meanLon);
    	setVariables(meanLat,meanLon);
    	String longZone = getLongZone(meanLon);
    	LatZones latZones = new LatZones();
    	String latZone = latZones.getLatZone(meanLat);
    	...
    	for(int i=0;i<coordinates.length;i++) {
    		latitude = coordinates[i].x;
    		longitude = coordinates[i].y;
    		setVariables(latitude, longitude);
    		
    		double _easting = getEasting();
    		double _northing = getNorthing(latitude);
    		
    		if (_easting < minEasting) {minEasting = _easting;};
    		if (_northing < minNorthing) {minNorthing = _northing;};
    		
    		out[i] = new Coordinate(_northing,_easting);
    	}
    	
    	HashMap<String,Object> result = new HashMap<String,Object>();
    	result.put("longZone", longZone);
    	result.put("latZone",latZone);
    	result.put("coordinates",out);
    	result.put("minEasting", minEasting);
    	result.put("minNorthing", minNorthing);
  ...
  private class UTM2LatLon
  {
    ...
    public Coordinate[] convertUTMToLatLong(Coordinate[] utmCoordinates, String latZone, String longZone)
    {
    	Coordinate[] result = new Coordinate[utmCoordinates.length];
    	zone = Integer.parseInt(longZone);
...
//    	System.out.println(hemisphere);
    	...
    	for (int i=0;i<utmCoordinates.length;i++) {
    		northing = utmCoordinates[i].x;
        	easting = utmCoordinates[i].y;
    		...
    		result[i] = new Coordinate(latitude,longitude);
